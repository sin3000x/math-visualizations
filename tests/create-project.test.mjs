...
test('创建项目登记 workspace、更新锁文件，并拒绝覆盖或重复包名', async () => {
  ...
  try {
    ...
    assert.deepEqual(manifest.workspaces, ['packages/scene-kit', 'templates/scene-project', 'projects/linear-map']);
    const lock = JSON.parse(await readFile(path.join(root, 'package-lock.json')));
    assert.equal(lock.packages['projects/linear-map'].version, '0.1.0');
    assert.equal(JSON.parse(await readFile(path.join(root, 'projects/linear-map/package.json'))).name, 'linear-map');
    assert.equal(lock.packages['node_modules/linear-map'].link, true);
    assert.equal(await readFile(path.join(root, 'projects/linear-map/README.md'), 'utf8'), '[规范](../../AGENTS.md)');
    for (const item of ['node_modules', 'dist', 'exports', 'package-lock.json']) {
      await assert.rejects(access(path.join(root, 'projects/linear-map', item)));
    }
    const before = await readFile(path.join(root, 'package-lock.json'), 'utf8');
    await mkdir(path.join(root, 'projects/existing'));
    for (const name of ['linear-map', 'scene-project', '../outside', 'existing']) {
    ...
    const imported = spawnSync(process.execPath, ['projects/linear-map/check.mjs'], { cwd: root, encoding: 'utf8', timeout: 10_000 });
