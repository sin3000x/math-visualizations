...
后续构建首先遵循根目录 [默认构建偏好](../../AGENTS.md#默认构建偏好从-dual-space-提炼)：极简、图形优先、少公式、无标题和说明、无可见导航，以键盘触发动画渐进展现。下面描述模板当前实现；示例中现有的标签和工具栏不代表必须保留的设计，应在构建新主题时按该偏好对齐。

```sh
npm run dev
```

先在仓库根目录执行 `npm ci`，再进入本项目运行上述命令。需要 Node.js >= 22.13。端口以 Vite 输出为准；可用 `npm run dev -- --port 4180` 指定端口。
...
普通模式采用简单深色背景，画布外保留 Scene 标签和“全屏录制”；Scene 名称只在标签中出现一次。画布纯黑，内部用 **1440×810** 坐标设计，由 `.scene-design` 一次等比缩放到 16:9 外框。设计契约和验收视口为 **1920×1080**。

Scene 内容可用区域为 1440×810（留白由 Scene 自己安排），不为底部导航预留空间。Scene 内使用固定尺寸或相对父容器的尺寸，不用 `vw/vh` 重排内容；不要自行再缩放整个 Scene。窄屏只缩小画布，不改变构图。

普通模式的场景导航位于画布外顶部工具栏，当前场景高亮；步骤通过键盘切换。普通模式和录屏模式的页面底部及画布内均不显示导航栏、步骤按钮或页码。全屏录制隐藏外部工具栏和 Scene 标签，通过键盘导航；浏览器拒绝原生全屏时保留 CSS 录屏模式。按 Escape 退出。
...
本子项目保有自己的依赖声明和源码，安装与锁文件由根目录 npm workspaces 统一管理；模板后续改动不会自动修改本项目。根目录 AGENTS.md 是统一工程规范。
