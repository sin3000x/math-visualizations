...
后续构建首先遵循根目录 [默认构建偏好](../../AGENTS.md#默认构建偏好从-dual-space-提炼)：极简、图形优先、少公式、无标题和说明、画布内无导航，以键盘触发动画渐进展现。下面描述模板当前实现；示例中现有的标签和工具栏不代表必须保留的设计，应在构建新主题时按该偏好对齐。
...
本子项目保有自己的依赖声明和源码，安装与锁文件由根目录 npm workspaces 统一管理；模板中的教学示例改动不会自动修改本项目，共享组件和配置的修复会直接生效。根目录 AGENTS.md 是统一工程规范。
